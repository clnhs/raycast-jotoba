...
export const parsePos = (
    unparsedPosData: string[] | PartOfSpeech,
    type?: string
): string => {
    const posDataBuffer = fs.readFileSync(
        `${environment.assetsPath}/PartsOfSpeech.json`
    );

    const { simple: simplePosData, detailed: detailedPosData } = JSON.parse(
        posDataBuffer.toString()
    );
            ...
            if (pos) {
                if (typeof posDetailData === "string") {
                    ...
                    Object.entries(posDetailData).map(detailData => {
                        ...
                        if (
                            potentialPosGroupMatches[posGroupName] &&
                            posGroupType
                        )
                            posMatch =
                                potentialPosGroupMatches[posGroupName].types[
                                    posGroupType
                                ];
                    });
                }
